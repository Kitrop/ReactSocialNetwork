...
const Pagination: FC<Props> = ({totalUsersCount, pageSize, currentPage, onPageChanged, portionSize = 15, filter}) => {
    ...
    useEffect(() => setPortionNumber(Math.ceil(currentPage / portionSize)), [currentPage, portionSize, portionCount])
    
    return <div>
        {/*First page*/}
        {portionCount !== 0 &&  <button className={styles.btn_pagination} onClick={() => setPortionNumber(portionNumber = 1)}>First</button>}
        {/*Prev button*/}
        ...
        {/*Next button*/}
        { portionCount > portionNumber && portionCount !== 0 && <button className={styles.btn_pagination} onClick={() =>  setPortionNumber(portionNumber + 1)}> NEXT </button> }
    </div>
}
